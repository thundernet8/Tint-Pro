...
<?php
	if(!empty($demos)){ ?>
		...
		<span class="demo">
			...
			?>
			<?php $demosuffix = ot_get_option('page_demo','demo'); $id = $post->ID.'_'.$i; $demopage = add_query_arg( 'id', $id, get_bloginfo('url').'/'.$demosuffix ) ; ?>
	...
	<?php if($dlmail==0||empty($dlmail)||$dlmail==1&&!empty($saledl)){ ?>
		...
		<span class="dl-link">
			<?php $dlsuffix = ot_get_option('page_dl','dl'); $pid = $post->ID; $dlpage = add_query_arg( 'pid', $pid, get_bloginfo('url').'/'.$dlsuffix ) ; ?>
